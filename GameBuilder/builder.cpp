...
void BuildProject()
{
    ...
    std::string outputFile_a = "exported_game/whatami.glsl";
    ...
    std::string outputFile_b = "exported_game/whereami.glsl";

    encryptFile(inputFile_b, outputFile_b, key_b);


    const char* compileCommand = R"""(
    cp project.json exported_game && cp -r project/ exported_game && cd GameBuilder && make build 
    )""";
