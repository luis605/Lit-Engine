...
std::vector<Vector3> GetNearbyVertices(const std::unordered_map<Vector3, Vector3, Vector3Hash, Vector3Equal>& vertexGridID, const Vector3& targetVertex, float gridSize) {
    ...
    if (gridSize <= 0.0f) return nearbyVertices;

    // Calculate the grid position of the target vertex
    int gridX = static_cast<int>(std::floor(targetVertex.x / gridSize));
    int gridY = static_cast<int>(std::floor(targetVertex.y / gridSize));
    int gridZ = static_cast<int>(std::floor(targetVertex.z / gridSize));
...
int main() {
    ...
    float gridSize = 1.f; // Adjust the grid size as needed
