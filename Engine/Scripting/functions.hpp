...
PYBIND11_EMBEDDED_MODULE(mathModule, m) {
    ...
    m.def("lerp", static_cast<float(*)(float, float, float)>(&litLerp<float>), "Lerp function for float and double types");
    m.def("lerp", static_cast<int(*)(int, int, float)>(&lerpInt), "Lerp function for integer types");
