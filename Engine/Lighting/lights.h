...
struct SurfaceMaterialTexture {
  ...
  ~SurfaceMaterialTexture() {
    // cleanup();  // Commented: Would be called when not needed. Happened when
    // adding a new entity
  }

  SurfaceMaterialTexture &operator=(const SurfaceMaterialTexture &other) {
    if (this != &other) {
      // cleanup(); // Commented: Would be called when not needed. Requires
      // review. Happened when running game (assign operator) and when adding a
      // new entity
  ...
  bool hasTexture() const { return IsTextureReady(staticTexture); }

  // Check if it has a video
  const bool hasVideo() const { return videoTexture.hasVideoStream(); }

  // Check if it is empty
  const bool isEmpty() const { return activatedMode == -1; }

  // Get texture
  const Texture2D &getTexture2D() const { return staticTexture; }

  // Get video
  Video &getVideo() { return videoTexture; }

  void updateVideo() { videoTexture.PlayCurrentFrame(); }

  Texture2D &getVideoTexture() { return videoTexture.getTexture(); }
