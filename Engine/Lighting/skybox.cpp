...
void Skybox::loadSkybox(const fs::path& texturePath) {
    ...
    SetShaderValue(
        ...
        GetUniformLocation(cubeModel.materials[0].shader, "environmentMap"),
        temp_array1, SHADER_UNIFORM_INT);
    SetShaderValue(cubeModel.materials[0].shader,
                   GetUniformLocation(cubeModel.materials[0].shader, "doGamma"),
                   useHDR, SHADER_UNIFORM_INT);
    SetShaderValue(
        cubeModel.materials[0].shader,
        GetUniformLocation(cubeModel.materials[0].shader, "vflipped"), useHDR,
        SHADER_UNIFORM_INT);

    int array0[1] = {0};
    SetShaderValue(cubemapShader,
                   GetUniformLocation(cubemapShader, "equirectangularMap"),
...
void Skybox::updateBuffer() {
    ...
    for (int index = 0; index < objectsCount; ++index) {
        ...
        SetShaderValueTexture(
            skyboxShader,
            GetUniformLocation(skyboxShader, textureUniformName.c_str()),
            object.texture);

        SetShaderValue(
            skyboxShader,
            GetUniformLocation(skyboxShader,
    ...
    SetShaderValue(skyboxShader,
                   GetUniformLocation(skyboxShader, "objectsCount"),
...
void Skybox::setExposure(float skyboxExposure) {
    SetShaderValue(this->skyboxShader,
                   GetUniformLocation(skyboxShader, "skyboxExposure"),
