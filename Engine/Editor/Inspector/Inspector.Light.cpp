...
void LightInspector()
{
    ...
    if (ImGui::CollapsingHeader(ICON_FA_SLIDERS " Light Properties", false)) {
        ...
        if (!AttenuationActiveInputMode) {
            ImGui::SliderFloat("Attenuation", &selectedLight->light.aisr.x, 0.0f, 0.3f);
            AttenuationActiveInputMode = ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0);
        } else {
            if (ImGui::InputFloat("Attenuation", &selectedLight->light.aisr.x, 0.0f, 1.0f, "%.3f", ImGuiInputTextFlags_EnterReturnsTrue))
                AttenuationActiveInputMode = false;
        }

        if (!IntensityActiveInputMode) {
            ImGui::SliderFloat("Intensity", &selectedLight->light.aisr.y, 0.0f, 50.0f);
            IntensityActiveInputMode = ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0);
        } else {
            if (ImGui::InputFloat("Intensity", &selectedLight->light.aisr.y, 0.0f, 100000.0f, "%.3f", ImGuiInputTextFlags_EnterReturnsTrue))
                IntensityActiveInputMode = false;
        }

        if (!SpecularStrenghtActiveInputMode) {
            ImGui::SliderFloat("Specular Strenght", &selectedLight->light.aisr.z, 0.0f, 1.0f);
            SpecularStrenghtActiveInputMode = ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0);
        } else {
            if (ImGui::InputFloat("Specular Strenght", &selectedLight->light.aisr.z, 0.0f, 1.0f, "%.3f", ImGuiInputTextFlags_EnterReturnsTrue))
                SpecularStrenghtActiveInputMode = false;
        }

        if (!RadiusActiveInputMode) {
            ImGui::SliderFloat("Radius", &selectedLight->light.aisr.w, 0.0f, 100.0f);
            RadiusActiveInputMode = ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0);
        } else {
            if (ImGui::InputFloat("Radius", &selectedLight->light.aisr.w, 0.0f, 100000.0f, "%.3f", ImGuiInputTextFlags_EnterReturnsTrue))
