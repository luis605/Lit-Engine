...
void  Application::update() {
   ...
   if (InputManager::IsKeyPressed(GLFW_KEY_F10)) m_engine.profiler().setEnabled(!m_engine.profiler().enabled());
   if (InputManager::IsKeyPressed(GLFW_KEY_F11) && m_engine.profiler().writeChromeTrace("resources/trace.json")) Lit::Log::Info("Profiler trace written to resources/trace.json");
   if (m_engine.profiler().enabled()) {
       float profileY = 390.0f;
       m_engine.AddText(std::format("CPU profile (F10 off, F11 trace)  frame {:.2f} ms", m_engine.profiler().lastFrameMs()), 900.0f, profileY, 0.45f, glm::vec3(0.4f, 1.0f, 0.6f));
       for (const ProfileSummary& scope : m_engine.profiler().topScopes(8)) {
           profileY -= 18.0f;
           m_engine.AddText(std::format("{}: {:.3f} ms x{}", scope.name, scope.totalMs, scope.calls), 900.0f, profileY, 0.45f, glm::vec3(1.0f));
       }
   }
